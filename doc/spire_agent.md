...
| `enable_wit_svids`            | Enable WIT-SVID support. When enabled, the agent will cache and rotate WIT-SVIDs for workloads that subscribe to them.                                                              | false                   |
| `wit_svid_cache_max_size`     | Soft limit of max number of WIT-SVIDs that would be stored in the LRU cache. Only relevant when `enable_wit_svids` is true.                                                         | 1000                    |
