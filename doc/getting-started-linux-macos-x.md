...
Download the tarball for your platform from the [SPIRE releases page](https://github.com/spiffe/spire/releases/latest) and unpack the pre-built `spire-server` and `spire-agent` executables and example configuration files into a `spire-<version>` directory, substituting `<version>` and `<asset>` for the release version and asset name you downloaded:

```
$ curl -s -N -L https://github.com/spiffe/spire/releases/download/<version>/<asset>.tar.gz | tar xz
```

## Building SPIRE on macOS/Darwin

Run the following commands to clone SPIRE and build the `spire-server` and `spire-agent` executables using go. The executables will be created in the `spire` directory. Replace `main` with a specific [release tag](https://github.com/spiffe/spire/releases) if you want to build a particular version.

*To complete this step you will need Go 1.13 or higher ([https://golang.org/dl/](https://golang.org/dl/))*

```
$ git clone --single-branch --branch main https://github.com/spiffe/spire.git
$ cd spire
...
> **Note**
> A Join Token is just one of the many available agent attestation methods. To see a complete list of available attestors, click [here](registering.md#1-defining-the-spiffe-id-of-the-agent).
...
> **Note**
> `unix` is just one of the available workload attestation methods. To see a complete list of available attestors, click [here](registering.md#2-defining-the-spiffe-id-of-the-workload).
...
* [Review the SPIRE Documentation](configuring.md) to learn how to configure SPIRE for your environment.
