...
By default the Server expects the configuration file to reside at `conf/server/server.conf`, however the Server can be configured to use a configuration file in a different location with the `--config` flag. See the [SPIRE Server reference](spire_server.md) for more information.

Similarly, the Agent expects this file to reside at `conf/agent/agent.conf`, however the Agent can be configured to use a configuration file in a different location with the `--config` flag. See the [SPIRE Agent reference](spire_agent.md) for more information.
...
A SPIFFE Server identifies and attests Agents through the process of *node attestation* (read more about this in [SPIRE Concepts](https://spiffe.io/docs/latest/spire-about/spire-concepts/)). This is accomplished through Node Attestor plugins, which you configure and enable in the server. 
...
## Attestation of nodes running Kubernetes

To issue identities to workloads running in a Kubernetes cluster, it is necessary to deploy a SPIRE Agent to each node in that cluster that is running a workload ([read more](install-agents.md#installing-spire-agents-on-kubernetes) on how to install SPIRE Agents on Kubernetes).
...
To use PSAT Node Attestation, configure enable the PSAT Node Attestor plugin on the [SPIRE Server](plugin_server_nodeattestor_k8s_psat.md) and [SPIRE Agent](plugin_agent_nodeattestor_k8s_psat.md).

> **Note**
> SAT based node attestation, an earlier alternative to PSAT, is no longer supported as of SPIRE 1.12.0.

## Attestation of nodes running Linux
...
Once join token node attestation has been configured, a join token can be generated on the server using the `spire-server token generate` command. Optionally you can associate a particular SPIFFE ID with the Join Token with the `-spiffeID` flag. [Read more](spire_server.md#spire-server-token-generate) about using this command.

When starting a SPIRE Agent for the first time with Join Token attestation enabled, the agent can be started with the `spire-agent run` command, and specifying the join token generated by the server using the `-joinToken` flag. [Read more](spire_agent.md#spire-agent-run) about this command.
...
To use Join Token Node Attestation, configure and enable the join token Node Attestor plugin on the [SPIRE Server](plugin_server_nodeattestor_jointoken.md) and [SPIRE Agent](plugin_agent_nodeattestor_jointoken.md).
...
To use X.509 Certificate Node Attestation, configure and enable the x509pop Node Attestor plugin on the [SPIRE Server](plugin_server_nodeattestor_x509pop.md) and [SPIRE Agent](plugin_agent_nodeattestor_x509pop.md).
...
To use SSH Certificate Node Attestation, configure and enable the sshpop Node Attestor plugin on the [SPIRE Server](plugin_server_nodeattestor_sshpop.md) and [SPIRE Agent](plugin_agent_nodeattestor_sshpop.md).

## Attestation for Linux nodes on a Cloud Provider
...
To use GCP IIT Node Attestation, configure and enable the gcp_iit Node Attestor plugin on the [SPIRE Server](plugin_server_nodeattestor_gcp_iit.md) and [SPIRE Agent](plugin_agent_nodeattestor_gcp_iit.md).
...
For more information on configuring AWS EC2 Node Attestor plugins, refer to the corresponding SPIRE documentation for the AWS [SPIRE Server Node Attestor](plugin_server_nodeattestor_aws_iid.md) on the SPIRE Server and the [SPIRE Agent Node Attestor](plugin_agent_nodeattestor_aws_iid.md) on the agent.
...
For more information on configuring Azure MSI Node Attestor plugins, refer to the corresponding SPIRE documentation for the Azure MSI [SPIRE Server Node Attestor](plugin_server_nodeattestor_azure_msi.md)  on the SPIRE Server and the [SPIRE Agent Node Attestor](plugin_agent_nodeattestor_azure_msi.md) on the agent.
...
For more information, including details of the exposed selectors, refer to the corresponding SPIRE documentation for the [Kubernetes Workload Attestor plugin](plugin_agent_workloadattestor_k8s.md).
...
For more information, including details of the exposed selectors, refer to the corresponding SPIRE documentation for the [Docker Workload Attestor plugin](plugin_agent_workloadattestor_docker.md).
...
For more information, including details of the exposed selectors, refer to the corresponding SPIRE documentation for the [Unix Workload Attestor plugin](plugin_agent_workloadattestor_unix.md).
...
The SPIRE Server can be configured to utilize different SQL-compatible storage backends by configuring the default SQL data-store plugin as described below. A complete reference for how this block is configured can be found in the [SPIRE documentation](plugin_server_datastore_sql.md).
...
For a complete server configuration reference, see the [SPIRE Server Configuration Reference](spire_server.md).
...
This strategy can be managed by enabling and configuring the `disk` UpstreamAuthority plugin for the [SPIRE Server](plugin_server_upstreamauthority_disk.md).
...
This strategy can be managed by enabling and configuring the `awssecret` UpstreamAuthority plugin for the [SPIRE Server](plugin_server_upstreamauthority_awssecret.md).
...
This strategy can be managed by enabling and configuring the `aws_pca` UpstreamAuthority plugin for the [SPIRE Server](plugin_server_upstreamauthority_aws_pca.md).
...
A full treatment for Nested SPIRE is beyond the scope of this guide. However this strategy can be managed by enabling and configuring the `spire` UpstreamAuthority plugin for the [SPIRE Server](plugin_server_upstreamauthority_spire.md).
...
* **Kubernetes ConfigMap**: Publishes the trust bundle to Kubernetes ConfigMaps, making it available to workloads running in Kubernetes clusters. This is managed by enabling and configuring the `k8s_configmap` BundlePublisher plugin for the [SPIRE Server](plugin_server_bundlepublisher_k8s_configmap.md).

* **AWS S3**: Publishes the trust bundle to an Amazon S3 bucket, making it accessible to workloads that can read from S3. This is managed by enabling and configuring the `aws_s3` BundlePublisher plugin for the [SPIRE Server](plugin_server_bundlepublisher_aws_s3.md).

* **AWS IAM Roles Anywhere Trust Anchor**: Publishes the trust bundle to an AWS IAM Roles Anywhere trust anchor, enabling workloads with SPIFFE identities to obtain AWS IAM credentials. This is managed by enabling and configuring the `aws_rolesanywhere_trustanchor` BundlePublisher plugin for the [SPIRE Server](plugin_server_bundlepublisher_aws_rolesanywhere_trustanchor.md).

* **GCP Cloud Storage**: Publishes the trust bundle to a Google Cloud Storage bucket, making it accessible to workloads that can read from GCS. This is managed by enabling and configuring the `gcp_cloudstorage` BundlePublisher plugin for the [SPIRE Server](plugin_server_bundlepublisher_gcp_cloudstorage.md).
...
For more information, see the [telemetry configuration](telemetry/telemetry_config.md) guide.
...
Once you've configured your Server and Agents, consider reviewing the guide on [Registering Workloads](registering.md).
