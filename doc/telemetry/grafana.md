...
The Compose setup in this directory starts SPIRE Server, SPIRE Agent,
Prometheus, and Grafana, and provisions this dashboard automatically. It needs
...
Open [Grafana](http://localhost:3000) to view and edit the dashboard. Prometheus
is also available at [localhost:9090](http://localhost:9090); its
`Status > Target health` page shows both SPIRE scrape targets.

The setup uses the repository's `spire-server:latest-local` and
`spire-agent:latest-local` images by default, so locally built changes can be
inspected. If they are already built, the `make images` step can be skipped.
Override `SPIRE_SERVER_IMAGE` and `SPIRE_AGENT_IMAGE` to use other image names
or published versions.

Stop the services with `docker compose down`. Add `--volumes` to discard all
SPIRE, Prometheus, and Grafana state and start clean the next time. This is a
local development setup: it uses join-token attestation with insecure
bootstrapping, and its ports are bound only to localhost.
...
1. **Connecting Grafana to Prometheus:** Ensure Grafana is configured to use Prometheus as a data source (data source UID: `uid_prometheus_grafana_datasource`)
