...
func (v1 *V1) OpenUpstreamAuthorityStream(ctx context.Context) (_ []*x509certificate.X509Authority, _ []*common.PublicKey, _ UpstreamAuthorityStream, err error) {
	ctx, cancel := context.WithCancel(ctx)
	defer func() {
		// Only cancel the context if the function fails. Otherwise, the
		// returned stream will be in charge of cancellation.
	...
	stream, err := v1.UpstreamAuthorityPluginClient.SubscribeToAuthorityUpdates(ctx, &upstreamauthorityv1.SubscribeToAuthorityUpdatesRequest{})
	if err != nil {
		return nil, nil, nil, v1.WrapErr(err)
	}
	...
	return x509Authorities, jwtKeys, &v1UpstreamAuthorityStream{v1: v1, stream: stream, cancel: cancel}, nil
}
...
type v1UpstreamAuthorityStream struct {
	v1     *V1
	stream upstreamauthorityv1.UpstreamAuthority_SubscribeToAuthorityUpdatesClient
	cancel context.CancelFunc
}

func (s *v1UpstreamAuthorityStream) RecvUpstreamAuthorities() ([]*x509certificate.X509Authority, []*common.PublicKey, error) {
	for {
		resp, err := s.stream.Recv()
		switch {
		case errors.Is(err, io.EOF):
...
func (s *v1UpstreamAuthorityStream) Close() {
	s.cancel()
}
